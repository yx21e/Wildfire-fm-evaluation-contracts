...
\caption{Example \model{} active-fire occupancy overlay generated from 2026-07-06 12:00 UTC HRRR input, with a 12-hour lead to 2026-07-07 00:00 UTC. The example applies USGS-consistency calibration using same-day USGS Fire Danger Forecast surfaces~\cite{usgs_fire_danger_forecast}. The rendered image uses a sparse green-dominant discrete speckled raster display: cell-level categorical marks, deterministic micro-texture, display omissions for low-salience cells, fragmented warm-color pockets, and no Gaussian blur or smooth interpolation. Quantitative analysis should use the probability raster.}
...
It converts the calibrated probability field into sparse categorical cell marks with deterministic jitter, micro-clusters, short streaks, fragmented edges, display omissions, and a green-to-yellow-to-orange-to-red wildfire-risk palette.
This rendering makes local spatial texture visible in the preview without turning the map into a smooth continuous heatmap; it does not change the probability raster.
Areas without overlay in the preview are visual omissions for low-salience cells rather than missing probability values.
