...
\caption{Example \model{} active-fire occupancy heatmap generated from 2026-07-06 12:00 UTC HRRR input, with a 12-hour lead to 2026-07-07 00:00 UTC. The example applies USGS-consistency calibration using same-day USGS Fire Danger Forecast surfaces~\cite{usgs_fire_danger_forecast}. Brighter colors indicate higher displayed active-fire occupancy probability within the visualization transform. The rendered image uses adaptive display scaling for readability; quantitative analysis should use the probability raster.}
...
The rendered preview in Figure~\ref{fig:example_heatmap} uses a display-only transform that blends global probability scaling with longitude-band local contrast.
This transform makes low-probability regional structure, including eastern U.S. spatial variation, visible in the preview; it does not change the probability raster.
...
Rendered heatmap & Provides an adaptive visual preview of the same probability output. \\
